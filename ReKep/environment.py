...
class ReKepOGEnv:
    ...
    def save_video(self, save_path=None):
        ...
        for rgb in self.video_cache:
            ...
            """
            video_writer.append_data(to_numpy(rgb))
        video_writer.close()
        return save_path
    ...
    def _check_reached_ee(self, target_pos, target_xyzw, pos_threshold, rot_threshold):
        ...
        """
        Convert Current Pose to numpy array
        """
        current_pos = to_numpy(self.robot.get_eef_position())
        current_xyzw = to_numpy(self.robot.get_eef_orientation())
    ...
    def _move_to_waypoint(self, target_pose_world, pos_threshold=0.02, rot_threshold=3.0, max_steps=10):
        ...
        while count < max_steps:
            ...
            Convert Current Pose to numpy array
            """
            relative_position = target_pose_robot[:3, 3] - to_numpy(self.robot.get_relative_eef_position())
            relative_quat = T.quat_distance(T.mat2quat(target_pose_robot[:3, :3]), to_numpy(self.robot.get_relative_eef_orientation()))
