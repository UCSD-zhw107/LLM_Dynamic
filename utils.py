...
def to_tensor(data):
    if isinstance(data, np.ndarray):
        return torch.tensor(data, dtype=torch.float32, device='cuda' if torch.cuda.is_available() else 'cpu')
    elif isinstance(data, (list, float, int)):
        return torch.tensor(data, dtype=torch.float32, device='cuda' if torch.cuda.is_available() else 'cpu')
    elif isinstance(data, torch.Tensor):
        return data.to(dtype=torch.float32, device='cuda' if torch.cuda.is_available() else 'cpu')
    else:
